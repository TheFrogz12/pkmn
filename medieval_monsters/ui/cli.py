...
from medieval_monsters.systems import combat, exploration, survival
...
def run_cli() -> None:
    ...
    while True:
        ...
        if action == "help":
            ...
            for note in exploration.resolve_event(player, current_region, event, rng=rng):
                print(f"* {note}")
            if event.outcome == "encounter":
                active_monster = player.active_monster()
                if not active_monster:
                    print("You have no monsters able to fight!")
                    continue
                wild_monster = rng.choice(monsters).copy()
                battle = combat.run_auto_battle(active_monster, wild_monster, rng=rng)
                for entry in battle.log:
                    if entry.event == "capture":
                        chance_pct = int(round((entry.chance or 0.0) * 100))
                        outcome = "success" if entry.success else "fails"
                        print(f"Capture attempt ({chance_pct}% chance) {outcome}.")
                    else:
                        skill_text = f" using {entry.skill}" if entry.skill else ""
                        print(
                            f"{entry.actor} strikes {entry.target}{skill_text} for {entry.damage} damage "
                            f"({entry.remaining_hp} HP left)."
                        )
                if battle.captured:
                    print(f"You captured {wild_monster.name}!")
                    wild_monster.reset()
                    try:
                        player.add_to_party(wild_monster)
                        print(f"{wild_monster.name} joins your party.")
                    except ValueError:
                        print("Your party is full, so the captured monster will await you at camp.")
                else:
                    winner = battle.winner or "No one"
                    print(f"{winner} wins the battle.")
                    if active_monster.is_fainted:
                        print(f"{active_monster.name} needs rest after the defeat.")
        else:
